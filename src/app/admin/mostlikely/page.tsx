...
import AddMostLikelyModal from '@/app/components/admin/AddMostLikelyModal';
...
export default function MostLikelyAdminPage() {
    const [statements, setStatements] = useState<MostLikelyStatement[]>([]);
    const [loading, setLoading] = useState(true);
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<MostLikelyStatement | null>(null);
    ...
    const fetchStatements = async () => {
        ...
        if (error) {
            ...
            setStatements((data as unknown as MostLikelyStatement[]) ?? []);
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        const { error } = await supabase.from('most_likely_statements').delete().eq('id', id);
        if (error) {
