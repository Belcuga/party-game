...
import { ArrowLeft, Pencil, Trash2, HeartHandshake } from 'lucide-react';
import Link from 'next/link';
import { WingmanPrompt } from '@/app/types/wingmanPrompt';
import AddWingmanPromptModal from '@/app/components/admin/AddWingmanPromptModal';
...
export default function WingmanAdminPage() {
    const [prompts, setPrompts] = useState<WingmanPrompt[]>([]);
    const [loading, setLoading] = useState(true);
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<WingmanPrompt | null>(null);
    ...
    const fetchPrompts = async () => {
        ...
        if (error) {
            ...
            setPrompts((data as unknown as WingmanPrompt[]) ?? []);
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        const { error } = await supabase.from('wingman_prompts').delete().eq('id', id);
        if (error) {
