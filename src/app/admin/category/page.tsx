...
import { ArrowLeft, Pencil, Trash2, Timer } from 'lucide-react';
...
export default function CategoryAdminPage() {
    ...
    const [editing, setEditing] = useState<CategoryPrompt | null>(null);
    ...
    const fetchPrompts = async () => {
        ...
        if (error) {
            ...
            setPrompts((data as unknown as CategoryPrompt[]) ?? []);
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        const { error } = await supabase.from('category_prompts').delete().eq('id', id);
        if (error) {
        ...
        <main className="h-screen overflow-hidden bg-gradient-to-br from-[#1a0142] via-[#2a064e] to-[#4b0c5e] text-white px-4 py-6 sm:py-10 flex flex-col items-center">
            ...
            <div className="w-full max-w-5xl flex-shrink-0 text-center mb-5">
                ...
                <p className="text-sm text-white/60 mt-1">
                    {loading ? 'Loading…' : `${prompts.length} categor${prompts.length === 1 ? 'y' : 'ies'}`}
                </p>
            </div>
                    ...
                    <div className="w-full h-full overflow-x-auto overflow-y-auto">
                        <table className="min-w-full border-collapse text-sm">
                            <thead className="sticky top-0 bg-[#1b003c] z-10">
                                <tr>
                                    <th className="p-3 border-b border-white/10 text-left text-white/60">Category</th>
                                    <th className="p-3 border-b border-white/10 text-left text-white/60">Count</th>
                            ...
                            <tbody>
                                {prompts.map((p) => (
                                    <tr key={p.id} className="hover:bg-white/5 transition-colors">
                                        <td className="p-3 border-b border-white/5 max-w-[350px]">{p.text}</td>
                                        <td className="p-3 border-b border-white/5 text-white/80">{p.count}</td>
