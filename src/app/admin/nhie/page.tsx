...
import { ArrowLeft, Pencil, Trash2, Hand } from 'lucide-react';
import Link from 'next/link';
import { NhieStatement } from '@/app/types/nhieStatement';
import AddNhieModal from '@/app/components/admin/AddNhieModal';
...
export default function NhieAdminPage() {
    const [statements, setStatements] = useState<NhieStatement[]>([]);
    const [loading, setLoading] = useState(true);
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<NhieStatement | null>(null);
    ...
    const fetchStatements = async () => {
        ...
        if (error) {
            ...
            setStatements((data as unknown as NhieStatement[]) ?? []);
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        const { error } = await supabase.from('nhie_statements').delete().eq('id', id);
        if (error) {
    ...
    return (
        <main className="h-screen overflow-hidden bg-gradient-to-br from-[#1a0142] via-[#2a064e] to-[#4b0c5e] text-white px-4 py-6 sm:py-10 flex flex-col items-center">
            <div className="w-full max-w-4xl flex-shrink-0 flex items-center justify-between mb-6">
            ...
            <div className="w-full max-w-4xl flex-shrink-0 text-center mb-5">
                <h2 className="text-xl font-semibold">Statements</h2>
                <p className="text-sm text-white/60 mt-1">
                    {loading ? 'Loading…' : `${statements.length} statement${statements.length === 1 ? '' : 's'}`}
                </p>
            </div>
            ...
            {loading ? (
                ...
                <div className="w-full max-w-4xl flex-1 min-h-0 rounded-2xl border border-white/10 bg-white/5 overflow-hidden">
                    <div className="w-full h-full overflow-x-auto overflow-y-auto">
                        <table className="min-w-full border-collapse text-sm">
                            <thead className="sticky top-0 bg-[#1b003c] z-10">
                                <tr>
                                    <th className="p-3 border-b border-white/10 text-left text-white/60">Statement</th>
                            ...
                            <tbody>
                                {statements.map((s) => (
                                    <tr key={s.id} className="hover:bg-white/5 transition-colors">
                                        <td className="p-3 border-b border-white/5 max-w-[400px]">{s.statement}</td>
                                        <td className="p-3 border-b border-white/5"><Pill value={s.dirty} /></td>
                                        <td className="p-3 border-b border-white/5 text-white/60">{s.like_count}</td>
                                        <td className="p-3 border-b border-white/5 text-white/60">{s.dislike_count}</td>
            ...
            <AddNhieModal
                ...
                existingStatement={editing || undefined}
            />
        </main>
    );
}
