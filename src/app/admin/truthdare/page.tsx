...
import AddTruthDareModal from '@/app/components/admin/AddTruthDareModal';
...
export default function TruthDareAdminPage() {
    const [prompts, setPrompts] = useState<TruthDarePrompt[]>([]);
    const [loading, setLoading] = useState(true);
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<TruthDarePrompt | null>(null);
    ...
    const fetchPrompts = async () => {
        ...
        if (error) {
            ...
            setPrompts((data as unknown as TruthDarePrompt[]) ?? []);
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        const { error } = await supabase.from('truth_dare_prompts').delete().eq('id', id);
        if (error) {
                    ...
                    <div className="w-full h-full overflow-x-auto overflow-y-auto">
                        <table className="min-w-full border-collapse text-sm">
                            <thead className="sticky top-0 bg-[#1b003c] z-10">
                                <tr>
                                    <th className="p-3 border-b border-white/10 text-left text-white/60">Type</th>
                            ...
                            <tbody>
                                {prompts.map((p) => (
                                    <tr key={p.id} className="hover:bg-white/5 transition-colors">
                                        <td className="p-3 border-b border-white/5 capitalize text-white/80">{p.type}</td>
