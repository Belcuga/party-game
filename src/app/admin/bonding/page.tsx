...
import { ArrowLeft, Pencil, Trash2, MessageCircleHeart } from 'lucide-react';
import Link from 'next/link';
import { BondingQuestion } from '@/app/types/bondingQuestion';
import AddBondingQuestionModal from '@/app/components/admin/AddBondingQuestionModal';
...
export default function BondingAdminPage() {
    const [questions, setQuestions] = useState<BondingQuestion[]>([]);
    const [loading, setLoading] = useState(true);
    const [modalOpen, setModalOpen] = useState(false);
    const [editing, setEditing] = useState<BondingQuestion | null>(null);
    ...
    const fetchQuestions = async () => {
        ...
        if (error) {
            ...
            setQuestions((data as unknown as BondingQuestion[]) ?? []);
        }
        setLoading(false);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        const { error } = await supabase.from('bonding_questions').delete().eq('id', id);
        if (error) {
    ...
    return (
        <main className="h-screen overflow-hidden bg-gradient-to-br from-[#1a0142] via-[#2a064e] to-[#4b0c5e] text-white px-4 py-6 sm:py-10 flex flex-col items-center">
                            ...
                            <tbody>
                                {questions.map((q) => (
                                    <tr key={q.id} className="hover:bg-white/5 transition-colors">
                                        <td className="p-3 border-b border-white/5 max-w-[450px]">{q.text}</td>
            ...
            <AddBondingQuestionModal
                ...
                existingQuestion={editing || undefined}
            />
        </main>
    );
}
